...
function RequestButton() {
  ...
  return (
    ...
    >
      ...
      <span>{pending ? 'Mengirim Kode OTP...' : 'Kirim Kode Verifikasi OTP'}</span>
    </button>
  )
}
...
function VerifyButton() {
  ...
  return (
    ...
    >
      ...
      <span>{pending ? 'Memverifikasi...' : 'Verifikasi OTP & Simpan Kata Sandi'}</span>
    </button>
  )
}
...
function ForgotPasswordContent() {
  ...
  // Determine active step and messages
  const activeStep =
    verifyState.step !== 'email'
      ? verifyState.step
      : requestState.step !== 'email'
      ? requestState.step
      : paramStep

  const effectiveEmail = requestState.email || currentEmail || paramEmail
  ...
  useEffect(() => {
    if (activeStep === 'otp') {
      setResendCooldown((prev) => (prev === 0 ? 60 : prev))
    }
  }, [activeStep])
  ...
  const passwordsMatch =
    newPasswordVal && confirmPasswordVal && newPasswordVal === confirmPasswordVal

  return (
    <div className="w-full max-w-md bg-white p-8 md:p-10 rounded-3xl shadow-xl border border-[#d5dcc4]/80 space-y-6">
      {/* Header Card Icon & Role Badge */}
      <div className="text-center space-y-2">
        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#f3f6e8] border border-[#d5dcc4] text-[11px] font-semibold text-[#3f5726] mb-1">
          <Sparkles className="w-3.5 h-3.5" />
          <span>Pemulihan Akun (Siswa & Guru BK)</span>
        </div>

        <div className="flex justify-center">
          <div className="inline-flex items-center justify-center w-12 h-12 rounded-2xl bg-[#f3f6e8] border border-[#d5dcc4] text-[#3f5726] shadow-xs">
            {activeStep === 'success' ? (
        ...
        <p className="text-xs md:text-sm text-[#2b3a1a]/70">
          {activeStep === 'success'
            ...
            : activeStep === 'otp'
            ? `Masukkan 6-digit kode verifikasi yang dikirim ke ${effectiveEmail}`
            : 'Masukkan alamat email akun Siswa atau Guru BK Anda untuk menerima 6-digit kode verifikasi OTP.'}
        </p>
      </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="siswa@sekolah.sch.id / guru@bk.sch.id"
              />
            </div>
            <p className="text-[11px] text-[#2b3a1a]/60">
              Kode OTP 6-digit akan dikirimkan ke alamat email ini.
            </p>
          </div>
        ...
        <form action={verifyAction} className="space-y-4">
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[11px] text-[#3f5726] font-semibold">
                Cek Email ({effectiveEmail})
              </span>
            </div>
            <div className="relative">
              <input
                ...
                className="block w-full px-4 py-3 text-center tracking-[0.4em] font-mono text-xl font-extrabold border-2 border-[#3f5726]/40 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#3f5726] focus:border-transparent text-[#1e2a14] bg-[#f3f6e8]/50 placeholder-[#2b3a1a]/20 transition-all"
                placeholder="123456"
              />
            </div>
          </div>
          ...
          <div className="flex items-center justify-between text-xs pt-2">
            <Link
              ...
              className="text-[#2b3a1a]/70 hover:text-[#2b3a1a] transition-colors cursor-pointer underline"
            >
              Ganti Email
            </Link>
            ...
            <button
              ...
              className="text-[#3f5726] font-semibold hover:underline flex items-center gap-1 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed disabled:no-underline"
            >
              <RefreshCw className={`w-3 h-3 ${resendCooldown > 0 ? '' : 'animate-spin-once'}`} />
              <span>
                {resendCooldown > 0
      ...
      {activeStep === 'success' && (
        <div className="space-y-6 text-center pt-2">
          <div className="p-4 bg-[#f3f6e8] rounded-2xl border border-[#d5dcc4] text-xs text-[#2b3a1a]/80 space-y-1">
            ...
            <p>Akun Anda (Siswa / Guru BK) kini siap digunakan dengan kata sandi baru.</p>
          </div>
  ...
  return (
    <div className="min-h-screen flex flex-col justify-between bg-[#f3f6e8] text-[#2b3a1a] selection:bg-[#c2db8f]/40 relative overflow-hidden">
      ...
      <main className="flex-1 flex items-center justify-center px-4 py-8 z-10">
        <Suspense fallback={<div className="w-full max-w-md bg-white p-8 rounded-3xl shadow-xl animate-pulse h-80" />}>
      ...
      <footer className="p-4 text-center text-xs text-[#2b3a1a]/50 z-10">
        <p>&copy; {new Date().getFullYear()} Mindfulnessintervention.id - Bimbingan Konseling & Pemulihan Akun Siswa & Guru BK.</p>
      </footer>
    </div>
  )
}
